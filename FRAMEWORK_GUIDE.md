...
| `/pm-diagrams` | When a visual is needed - 18 types (state, erd, domain, sequence, flow, bpmn, journey, wireflow, architecture, c4, dependency, kanban, storymap, gantt, jtbd, matrix, kano, swot). `/pm-diagrams [slug]` for a specific type, or no arg for a context-aware recommendation. Composition standard per type in `references/[slug].md`. |
...
| `/pm-root-cause` | When a live thing behaves unexpectedly - metric dropped, churn spiked, feature not adopted, conversion fell, tickets rising. **In-flight diagnostic engine** (distinct from pm-problem-validation which is foundational, and pm-stress-test which is adversarial): you describe the symptom, it drills to the real root cause. Structured flow: is it real or a measurement artifact → localize (segment/funnel/version) → what changed → candidate causes across categories → 5-Whys to an actionable cause → separate evidence from guess → cheapest confirm test. Baked-in: 11 diagnostic methods with a selection guide (5 Whys, Ishikawa, issue tree/MECE, change analysis, cohort/segmentation, funnel, Pareto, fault tree, DMAIC, Kepner-Tregoe, correlation-vs-causation/Simpson), a 12-anomaly candidate-cause differential library (cause · category · data signature · fastest test · frequency), a measurement-trap catalogue, a 12-item bias catalogue, and a pre-acceptance validation checklist. Hard rule: don't stop at the first plausible cause; correlation ≠ causation; symptom ≠ root. Two modes: guided investigation or quick differential. Ends with testable hypotheses that feed pm-hypotheses. |
