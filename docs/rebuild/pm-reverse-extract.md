...
4. **`mutex_tags` from real code (both standalone and reconciled mode)** - the shared modules/files each still-open feature touches, extracted directly from the codebase. Because the code exists, this is more accurate than a greenfield JIT guess, so the rebuild's first delivery plan (pm-stripe) has an accurate code-contention dimension immediately. This step runs even when `pm-reconcile features` delegates carding here - a reconcile-based rebuild that skips tagging leaves pm-stripe as blind to contention as greenfield (it will honestly mark those waves `⚠ projected parallelism`).
5. **Notion push** - Feature hierarchy pushed to Product Features database (if URL configured)
