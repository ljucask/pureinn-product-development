...
- **Feature Card at status ≥ `2_Spec_Done` without a `### Edge Case Coverage` table** (pre-5.62.0 card). Do NOT migrate it in the audit - that means writing acceptance criteria, which the audit must never invent. Route each card to `/pm-feature-design [FEAT-ID] --edge-cases`, ordered by status: `3_Ready_to_Build` and `4_In_Build` first, then `5_In_Review`, `2_Spec_Done` / `2b_In_Design`, and `6_Shipped` last. Severity: P2 for `2_Spec_Done`-`5_In_Review`; P3 for `6_Shipped` (a retro table only records gaps, it changes no shipped behavior - on a Rebuild workspace do not let dozens of these bury the active cards)
...
|---|---|
| **Structure** | Feature Cards match the canonical template (sections 1-4 + Subtasks; frontmatter: id, title, status, stripe, feature_set, actor, owner, priority, estimate, prd_ref, feature_flag, flag_default). Registers match current format and header. **Edge Case Coverage table** (Section 2, cards ≥ `2_Spec_Done`): exactly 6 rows with the canonical codes `EC-INPUT`, `EC-AUTH`, `EC-STATE`, `EC-CONC`, `EC-EXT`, `EC-CLIENT` (P2 - missing structure); no empty or TBD cell at status `3_Ready_to_Build`-`5_In_Review` (P1); an `N/A` with no reason or a generic one ("not relevant", "N/A") (P2, judgment call - ask, never supply the reason yourself); ACs written as a checkbox list instead of Given/When/Then (P3, cosmetic). |
...
| **Cross-references** | Every `BR-ID` / `TBL-ID` / entity link in a Feature Card resolves to the register. `feature_set` matches feature_list. `stripe` is a real stripe. **`prd_ref` resolves** - it must point at `product/PRD_master.md` (or an `/initiatives/[slug]/prd.md` for an Initiative PRD); a `prd_ref` still pointing at `/product/PRD.md` (or `product/PRD.md` / bare `PRD.md`) is a dead reference to the old PRD path and is drift (P1 - JIT design reads this file). **Edge Case Coverage refs resolve:** every `AC-NN` in the table exists in Section 2 and carries the matching `[EC-XXX]` tag in its title; every `OQ-` ID in the table exists in `domain/open_questions.md` - a non-existent reference is P0, same as a dangling BR-ID. No dangling links. |
