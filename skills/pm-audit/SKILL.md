...
| **Lifecycle** | `status` is one of the canonical 6 (`1_Backlog`..`6_Shipped`) **everywhere - frontmatter, feature_list, and Notion use the same single vocabulary**. No orphaned or invalid states. **Reconcile-reality words (`Built` / `In Progress` / `Backlog`) are NOT status values** - if a feature_list or Notion `Status` still carries them, that is drift: map to canonical (`Built`→`6_Shipped`, `In Progress`→`4_In_Build`, `Backlog`→`1_Backlog`) and keep the reality word only as a human label in the card's `Current state (extracted)` section. Mixed vocabularies on the status axis = P1. |
